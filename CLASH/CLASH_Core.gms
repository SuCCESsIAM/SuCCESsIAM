...
* Source: https://www.globalcarbonproject.org/nitrousoxidebudget/24/files/GCP_NitrousOxideBudget_2024.pdf
LU_EF_CropN2O(t,pool) = 4.6/15.8;
...
* Carbon stocks:
         ...
         LU_Cstock_ManagedLand(t)       Vegetation carbon stock of managed lands (GtC)
...
VARIABLES
         LU_NetCO2Terrestrial(t)    Net CO2 flux from land to atmosphere (Mt CO2 per year)
         LU_NetCO2LandUse(t)        Net CO2 emissions from land-use (Mt CO2 per year)
;
...
* Carbon stocks:
        ...
        EQ_LU_Cstock_ManagedLand(t)     Vegetation carbon stock of managed lands
        EQ_LU_NetCO2Terrestrial(t)      Net CO2 flux from land to atmosphere
        EQ_LU_NetCO2LandUse(t)          Net CO2 emission from land use
...
* Litter from harvests: total C stock of the cleared areas, minus harvested wood and residue C stock:
                                 + ( LU_Cdens(t,pool,'primf')*LU_clear_pri(t,pool) + sum(age, LU_Cdens_SecnFor(t,pool,age) * LU_clear_sec(t,pool,age)) )
                                 - LU_wood_Cdens(pool) * (LU_harv_wast(t,pool) + LU_harv_pulp(t,pool) + LU_harv_logs(t,pool)) / 1000
                                 - LU_Ccont * LU_harv_WoodResid(t,pool)/1000
                                )
;
...
EQ_LU_Cstock_ManagedLand(t)..
    LU_Cstock_ManagedLand(t) =E= sum(pool, LU_CStockVege(t,pool,'crops') + LU_CStockVege(t,pool,'pastr') + LU_CStockVege(t,pool,'secdf'))
;
    
EQ_LU_NetCO2LandUse(t)$(ord(t) LT card(t))..
    LU_NetCO2LandUse(t+1) =E= -1 * 44/12 * 1000 * (LU_Cstock_ManagedLand(t+1) - LU_Cstock_ManagedLand(t))/tstep
                                 + 44/12 * 1000 * sum(pool,LU_Cdens(t,pool,'primf') * LU_clear_pri(t,pool))
;


* Net land-use CO2 emissions per year (note conversion: GtC -> Mt CO2)
EQ_LU_NetCO2Terrestrial(t)$(ord(t) LT card(t))..
    LU_NetCO2Terrestrial(t+1) =E= -1 * 44/12 * 1000 * (
...
EQ_LU_harv_crops(t,pool)..                                           LU_harv_crops(t,pool) =L= 1000*LU_Cropping_Intensity*LU_Crop_Yields(t,pool) * LU_Area(t,pool,'crops');
...
* Harvests of different wood fractions (note: LU_v_xxxx is in m3/ha; LU_clear_sec in mln. km2)
EQ_LU_harv_wast(t,pool)..                                             LU_harv_wast(t,pool) =L= sum(age, LU_v_wast(t,pool,age) * 100*LU_clear_sec(t,pool,age)) + LU_v_wast_pr(t,pool) * 100*LU_clear_pri(t,pool);
EQ_LU_harv_pulp(t,pool)..                                             LU_harv_pulp(t,pool) =L= sum(age, LU_v_pulp(t,pool,age) * 100*LU_clear_sec(t,pool,age)) + LU_v_pulp_pr(t,pool) * 100*LU_clear_pri(t,pool);
EQ_LU_harv_logs(t,pool)..                                             LU_harv_logs(t,pool) =L= sum(age, LU_v_logs(t,pool,age) * 100*LU_clear_sec(t,pool,age)) + LU_v_logs_pr(t,pool) * 100*LU_clear_pri(t,pool);
...
LU_NetCO2Terrestrial.FX(tfirst(t)) = (1.1-3.1) * 44/12 * 1000;
* Similarly, fix the managed land net to the GCP 2021 value:
LU_NetCO2LandUse.FX(tfirst(t)) = 4100;
